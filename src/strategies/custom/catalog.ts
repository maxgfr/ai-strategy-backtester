...
export const catalog: Record<string, CatalogEntry> = {
  ...
  elderRay: {
    compute: (c, p) => elderRay(c, p.period ?? 13),
    params: [{ name: 'period', default: 13 }],
    outputType: 'object',
    fields: ['bullPower', 'bearPower'],
    defaultField: 'bullPower',
  },
  ravi: {
    compute: (c, p) => ravi(c, p.shortPeriod ?? 7, p.longPeriod ?? 65),
    params: [
      { name: 'shortPeriod', default: 7 },
      { name: 'longPeriod', default: 65 },
    ],
    outputType: 'number',
  },
  hma: {
    compute: (c, p) => hma(c, p.period ?? 16),
    params: [{ name: 'period', default: 16 }],
    outputType: 'number',
  },
  choppinessIndex: {
    compute: (c, p) => choppinessIndex(c, p.period ?? 14),
    params: [{ name: 'period', default: 14 }],
    outputType: 'number',
  },
  ultimateOscillator: {
    compute: (c, p) =>
      ultimateOscillator(c, p.period1 ?? 7, p.period2 ?? 14, p.period3 ?? 28),
    params: [
      { name: 'period1', default: 7 },
      { name: 'period2', default: 14 },
      { name: 'period3', default: 28 },
    ],
    outputType: 'number',
  },
  chaikinOscillator: {
    compute: (c, p) =>
      chaikinOscillator(c, p.fastPeriod ?? 3, p.slowPeriod ?? 10),
    params: [
  ...
  linearRegressionSlope: {
    compute: (c, p) => linearRegressionSlope(c, p.period ?? 14),
    params: [{ name: 'period', default: 14 }],
    outputType: 'number',
  },
  fisherTransform: {
    compute: (c, p) => fisherTransform(c, p.period ?? 10),
  ...
  coppockCurve: {
    ...
    params: [
      { name: 'rocPeriod1', default: 14 },
      { name: 'rocPeriod2', default: 11 },
      { name: 'wmaPeriod', default: 10 },
    ],
    outputType: 'number',
  },
  forceIndex: {
    compute: (c, p) => forceIndex(c, p.period ?? 13),
    params: [{ name: 'period', default: 13 }],
    outputType: 'number',
  },
  dpo: {
  ...
  vwma: {
    compute: (c, p) => vwma(c, p.period ?? 20),
    params: [{ name: 'period', default: 20 }],
    outputType: 'number',
  },
  rvi: {
    compute: (c, p) => rvi(c, p.period ?? 10),
    params: [{ name: 'period', default: 10 }],
    outputType: 'object',
    fields: ['rvi', 'signal'],
    defaultField: 'rvi',
  ...
  massIndex: {
    compute: (c, p) => massIndex(c, p.emaPeriod ?? 9, p.sumPeriod ?? 25),
    params: [
      { name: 'emaPeriod', default: 9 },
      { name: 'sumPeriod', default: 25 },
    ],
    outputType: 'number',
  },
  emv: {
    compute: (c, p) => emv(c, p.period ?? 14),
    params: [{ name: 'period', default: 14 }],
    outputType: 'number',
  },
  mcginleyDynamic: {
    compute: (c, p) => mcginleyDynamic(c, p.period ?? 14),
    params: [{ name: 'period', default: 14 }],
    outputType: 'number',
  },
}
